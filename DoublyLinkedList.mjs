...
class DoublyLinkedList {
    ...
    insertAt(index, data) {
        ...
        if(newNode.next == null){ //새로 삽입한 노드의 다음 노드가 null이라면, 즉 새로 삽입한 노드가 마지막 노드라면
            this.tail = newNode; //tail을 새로 삽입한 노드로 설정
        }
    ...
    deleteAt(index) { //함수의 매개변수로 인덱스만 받으면 제거할 수 있으므로 data는 필요없음
        ...
        if (index == 0) { //head노드를 제거하는 상황
            ...
            if(this.head.next == null){ //데이터가 하나 남은 경우
                ...
                this.head = this.head.next; //head의 노드가 제거 되므로 head의 다음노드를 새로운 head로 만든다
                this.head.prev = null;//새로 head가 된 노드의 이전 노드를 null로 설정해 연결을 끊어준다
            }
            this.count--;
            return deletedNode;
